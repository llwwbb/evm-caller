  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      ...
      <div className="flex flex-wrap items-center gap-3 border-b border-line bg-bg px-5 py-2.5 font-mono text-[11px]">
        <span className="text-[10px] uppercase tracking-[0.2em] text-fg-mute">method</span>
        ...
        <button
          ...
          disabled={loading}
          className="ml-auto rounded-sm bg-mint px-4 py-1.5 font-mono text-[11px] font-semibold text-bg hover:bg-mint/80 disabled:opacity-50"
        >
      ...
      <div className="grid flex-1 min-h-0" style={{ gridTemplateColumns: '2fr 3fr' }}>
        ...
        <div className="flex min-h-0 flex-col border-r border-line">
          ...
          <div className="flex-shrink-0 overflow-y-auto" style={{ maxHeight: '30%' }}>
            {accounts.length === 0 ? (
              <div className="p-5 font-ui text-[11px] text-fg-mute">
              ...
              accounts.map((a, i) => {
                ...
                return (
                  <div
                    ...
                    className={
                      'cursor-pointer border-b border-line-soft px-4 py-2 font-mono text-[11px] ' +
          ...
          <div className="border-t border-line bg-surface p-3">
            ...
            {savingPreset && (
              <div className="mb-2 flex gap-2">
                <input
                  ...
                  className="flex-1 rounded-sm border border-line bg-bg px-2 py-1 font-ui text-[11px] text-fg placeholder:text-fg-mute focus:border-mint focus:outline-none"
                />
                <button
                  onClick={handleSavePreset}
                  className="rounded-sm bg-mint px-3 py-1 font-mono text-[10px] text-bg"
                >
                  save
                </button>
              </div>
            ...
            <div className="max-h-[120px] space-y-0.5 overflow-y-auto">
              {presets.length === 0 ? (
                <div className="font-ui text-[11px] text-fg-mute">
                ...
                presets.map((p) => (
                  <div
                    key={p.id}
                    className="flex items-center gap-2 rounded-sm bg-bg px-2 py-1 font-mono text-[11px]"
                  >
                    <span
        ...
        <div className="flex min-h-0 flex-col">
          <div className="flex items-center gap-2 border-b border-line px-4 py-2 font-mono text-[10px]">
            ...
            <button
              ...
              className="ml-auto rounded-sm px-2 py-0.5 text-[10px] text-fg-dim hover:bg-surface-2 disabled:opacity-50"
            >
              + add call
            </button>
          </div>
              ...
              calls.map((c, idx) => {
                ...
                return (
                  <div key={c.id} className="border-b border-line-soft">
                    ...
                    >
                      <span className="text-[10px] text-fg-mute">#{idx}</span>
                      <span className="rounded-xs bg-call-blue/12 px-1.5 py-0.5 text-[9px] font-bold tracking-[0.08em] text-call-blue">
                        CALL
                      </span>
                          ...
                          <div>
                            ...
                            {fn.inputs.map((inp, i) => (
                              ...
                              >
                                <span className="truncate text-fg-mute">
                                  {inp.name || `arg${i}`}
                                  <span className="ml-1 text-[9px]">({inp.type})</span>
                                </span>
                                <input
                                  value={c.argValues[i] ?? ''}
