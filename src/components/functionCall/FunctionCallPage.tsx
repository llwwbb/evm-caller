...
const FunctionCallPage: React.FC<FunctionCallPageProps> = ({
  ...
  const recentCallsForFn = useMemo(() => {
    if (!selectedFn) return [];
    return callHistory.filter((h) => h.functionName === selectedFn.name);
  }, [callHistory, selectedFn]);
          ...
          <div className="flex min-h-0 flex-col overflow-y-auto">
            {!selectedFn ? (
              ...
              <>
                        ...
                        return (
                          ...
                          >
                            <div className="font-mono text-[10px]">
                              ...
                              <div className="text-[9px] text-fg-mute">{input.type}</div>
                            </div>
                            <input
                              value={values[i] ?? ''}
                ...
                <div className="flex-1 overflow-y-auto">
                  {recentCallsForFn.length === 0 ? (
                    <div className="p-5 font-ui text-[12px] text-fg-mute">
                      {t('functionCall.noRecentCalls')}
                    </div>
                  ) : (
                    recentCallsForFn.map((item) => {
                      const collapsed = collapsedResults.has(item.id);
                      const success = item.result.success;
                      return (
                        <div key={item.id} className="border-b border-line-soft">
                          <div
                            className="flex cursor-pointer items-center gap-2 bg-surface px-5 py-2 font-mono text-[10px]"
                            onClick={() => toggleResultCollapse(item.id)}
                          >
                            <span
                              className={
                            ...
                            {item.rpcName && (
                              <span className="text-fg-mute">· {item.rpcName}</span>
                            )}
                            {item.blockTag !== undefined && (
                              <span className="text-fg-mute">· block {item.blockTag}</span>
                            )}
