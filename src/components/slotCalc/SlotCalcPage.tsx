...
const ModeBtn: React.FC<{
  ...
  <button
    onClick={onClick}
    className={
      'rounded-sm px-2.5 py-1 text-[12px] ' +
  ...
  return (
    <div className="grid flex-1 min-h-0" style={{ gridTemplateColumns: '2fr 3fr' }}>
      <div className="flex min-h-0 flex-col border-r border-line p-5">
        <div className="mb-2 font-mono text-[11px] uppercase tracking-[0.22em] text-fg-mute">
          {t('slotCalc.variable')}
        </div>
        <div className="space-y-2 font-mono text-[13px]">
            ...
            <div className="mt-3">
              ...
              {Array.from({ length: expectedKeyCount }).map((_, i) => (
                <input
                  ...
                  placeholder={`key[${i}]`}
                  className="mb-1 w-full rounded-sm border border-line bg-bg px-2 py-1 font-mono text-[13px] text-fg placeholder:text-fg-mute focus:border-mint focus:outline-none"
                />
              ))}
            </div>
          )}
          ...
          {isArray && (
            <div className="mt-3">
              ...
              <input
                ...
                placeholder="0"
                className="w-full rounded-sm border border-line bg-bg px-2 py-1 font-mono text-[13px] text-fg placeholder:text-fg-mute focus:border-mint focus:outline-none"
              />
            </div>
          )}
          ...
          <button
            ...
            className="mt-2 w-full rounded-sm bg-mint px-3 py-1.5 font-mono text-[13px] font-semibold text-bg hover:bg-mint/80"
          >
            {t('slotCalc.computeSlot')}
          </button>

          {error && (
            <div className="mt-2 rounded-sm border border-call-red/30 bg-call-red/5 px-2.5 py-1.5 text-[12px] text-call-red">
              {error}
            </div>
          )}
        </div>
      ...
      <div className="flex min-h-0 flex-col p-5">
        ...
        {computedSlot ? (
          <div className="flex items-center gap-2 rounded-sm border border-line bg-bg px-3 py-2 font-mono text-[14px] text-mint">
            <span className="flex-1 break-all">{computedSlot}</span>
            <CopyBtn text={computedSlot} />
          </div>
        ) : (
          <div className="rounded-sm border border-line-soft bg-surface px-3 py-2 font-ui text-[13px] text-fg-mute">
            {t('slotCalc.computeHint')}
          </div>
        )}

        <div className="mt-6 mb-1 font-mono text-[11px] uppercase tracking-[0.22em] text-fg-mute">
        ...
        <div className="space-y-2 font-mono text-[13px]">
          ...
          <button
            ...
            className="rounded-sm border border-line px-3 py-1 text-[12px] text-fg-dim hover:bg-surface-2"
          >
            {t('slotCalc.encode')}
          </button>
          {encodedValue && (
            <div className="flex items-center gap-2 rounded-sm border border-line bg-bg px-3 py-2 text-[13px] text-fg">
        ...
        <div className="mt-6 rounded-sm border border-line-soft bg-surface p-3 font-ui text-[13px] leading-relaxed text-fg-dim">
          <div className="mb-1 font-mono text-[11px] uppercase tracking-[0.22em] text-fg-mute">
            {t('slotCalc.tips')}
          </div>
    ...
    <div className="grid flex-1 min-h-0" style={{ gridTemplateColumns: '2fr 3fr' }}>
      <div className="flex min-h-0 flex-col border-r border-line p-5">
        ...
        <textarea
          ...
          className="mb-2 min-h-[180px] flex-1 resize-none rounded-sm border border-line bg-bg px-2 py-1.5 font-mono text-[12px] text-fg placeholder:text-fg-mute focus:border-mint focus:outline-none"
        />
        <button
          onClick={handleParse}
          className="rounded-sm bg-mint px-3 py-1.5 font-mono text-[13px] font-semibold text-bg hover:bg-mint/80"
        >
          {t('slotCalc.parseLayout')}
        </button>
        {error && (
          <div className="mt-2 rounded-sm border border-call-red/30 bg-call-red/5 px-2.5 py-1.5 font-mono text-[12px] text-call-red">
            {error}
          </div>
        )}
      </div>
      ...
      <div className="flex min-h-0 flex-col overflow-y-auto p-5">
        {!vars ? (
          <div className="rounded-sm border border-line-soft bg-surface p-4 font-ui text-[13px] text-fg-mute">
            {t('slotCalc.jsonHint')}
          </div>
        ) : (
          <>
            ...
            <div className="mb-4 max-h-[200px] overflow-y-auto rounded-sm border border-line bg-bg">
              {vars.map((v, i) => (
                <div
                  ...
                  className={
                    'cursor-pointer border-b border-line-soft px-3 py-1.5 font-mono text-[13px] last:border-b-0 ' +
                ...
                >
                  ...
                  <span className="ml-2 text-[11px] text-fg-mute">
                    slot {v.slot}
            ...
            {selected && (
              <>
                ...
                {expectedKeyCount > 0 ? (
                  <div className="mb-2 space-y-1 font-mono text-[13px]">
                    {Array.from({ length: expectedKeyCount }).map((_, i) => (
                      <input
                        key={i}
                        value={keys[i] ?? ''}
                  ...
                  <div className="mb-2 font-mono text-[12px] text-fg-mute">
                    {t('slotCalc.simpleVar')}
                  </div>
                )}
                <button
                  onClick={handleCompute}
                  className="mb-2 rounded-sm bg-mint px-3 py-1.5 font-mono text-[13px] font-semibold text-bg hover:bg-mint/80"
                >
                  {t('slotCalc.computeSlot')}
                </button>
                {computeError && (
                  <div className="rounded-sm border border-call-red/30 bg-call-red/5 px-2.5 py-1.5 font-mono text-[12px] text-call-red">
                    {computeError}
                  </div>
                )}
                {computedSlot && (
...
const ProbeMode: React.FC<{ rpcUrl: string }> = ({ rpcUrl }) => {
  ...
  return (
    <div className="flex flex-1 min-h-0 flex-col p-5">
      <div className="mb-3 flex items-end gap-3 font-mono text-[13px]">
        ...
        <button
          onClick={handleFetch}
          disabled={loading}
          className="rounded-sm bg-mint px-4 py-1.5 font-mono text-[13px] font-semibold text-bg hover:bg-mint/80 disabled:opacity-50"
        >
      ...
      {error && (
        <div className="mb-3 rounded-sm border border-call-red/30 bg-call-red/5 px-2.5 py-1.5 font-mono text-[13px] text-call-red">
          {error}
        </div>
      )}

      {rows.length > 0 ? (
        <div className="flex-1 overflow-y-auto rounded-sm border border-line bg-bg font-mono text-[13px]">
          {rows.map((r, i) => {
                ...
                <div className="flex gap-0.5">
                  {(['hex', 'uint', 'address', 'bool'] as ProbeType[]).map((tp) => (
                    <button
                      ...
                      className={
                        'rounded-xs px-1.5 py-0.5 text-[11px] ' +
                        (type === tp
        ...
        <div className="flex flex-1 items-center justify-center text-center">
          <div>
            ...
            <p className="font-ui text-[14px] text-fg-mute">{t('slotCalc.probeHint')}</p>
          </div>
        </div>
      )}
    </div>
